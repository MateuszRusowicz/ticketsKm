...
| **7 Sep (Task 16 Step 7, MEASURED — root cause of "webhooks never fire")** | Local walkthrough produced **zero `StripeWebhookEvent` rows** and every order stuck at `paymentIntentStatus = 'requires_confirmation'`, despite `stripe listen` and `pnpm dev` both running and the `.env` `STRIPE_WEBHOOK_SECRET` matching `stripe listen --print-secret` exactly (verified by sha256). **Cause: the Stripe CLI is authenticated to a different account than the app.** `stripe config --list` → `acct_1UBpPOGTx5eWuMMc` ("test-tickets · sandbox"); `GET /v1/account` with the app's `STRIPE_SECRET_KEY` → `acct_1UBpQ6GVCpToPFr7`. The app creates PaymentIntents on one account while the CLI subscribes to events on the other, so real payments generate events the listener never sees. `stripe trigger` *did* produce rows — because it creates its PI on the CLI's own account. **This is the exact sandbox-vs-test-mode trap Task 1 Step 5 warned about; the warning was written but the check (`stripe config --list`) was evidently not run.** | Fix without re-login: `set -a; . ./.env; set +a` then `stripe listen --api-key "$STRIPE_SECRET_KEY" --forward-to localhost:3000/api/webhooks/stripe`, which pins the listener to the app's account and prints the matching `whsec_` for `.env`. Task 1 Step 5's check is now **mandatory before any walkthrough**, and Task 16 Step 7 gains a precondition: assert CLI account == key account. |
...
| **7 Sep (manual walkthrough defect, MEASURED)** | **`payment_intent.succeeded` never called `recordPaymentAttempt`** → `Order.paymentIntentStatus` stayed at `'requires_confirmation'` and `Order.paymentMethodType` stayed `null` after a successful card payment (measured on KM-2026-000007). The `processing`, `requires_action`, and `payment_failed` branches all called `recordPaymentAttempt`, so those fields were only ever written for non-card flows. Two pieces of evidence this was an oversight: (1) `prisma/schema.prisma` documents `paymentIntentStatus` as "updated by every subsequent webhook" — contradicted by the code; (2) the `succeeded` branch already performed `stripe.paymentIntents.retrieve(pi0.id, { expand: ['latest_charge'] })` with an explicit comment "so extractPaymentMethodType sees the full Charge object" — but then never called `extractPaymentMethodType`, making the Stripe round-trip pointless. **Fix:** widened `recordPaymentAttempt`'s `meta.reason` union to include `'succeeded'` in `orders.ts`; added `await recordPaymentAttempt(orderId, pi, { reason: 'succeeded' })` **before** `fulfilOrder` in `webhook-dispatch.ts`'s `succeeded` branch (order matters — persists the fact even when `fulfilOrder` throws or takes the refund branch). **Task 8 contract updated** (reason union); **Task 9 dispatch table updated** (`succeeded` row now includes the `recordPaymentAttempt` call). Test case 11 added to `webhook-dispatch.test.ts`; confirmed failing before fix and passing after. Negative control: reverting the call caused both case 1 and case 11 to fail. Suite: 404 tests / 45 files, all green. |

| **7 Sep (UX defect — form wipe on validation failure, FIXED)** | Submitting the checkout form with an invalid e-mail wiped every field — buyer had to retype name, all attendee names, and invoice details. **Cause:** `SubmitState` was `Record<string, never> \| { errors: Record<string, string[]> }` — the action returned only errors, never the submitted values. `<form action={…}>` submits natively; after the action resolves React re-renders with uncontrolled inputs falling back to react-hook-form's empty `defaultValues`. **Fix (two parts):** (1) `SubmitState` extended with `values?: Partial<CheckoutInput>`; `submitCheckout` builds `echoValues` from `FormData` before any early return and includes it on all six error paths (rate limit, attendee-name gap, Zod field errors, soldOut, aboveMax, notPurchasable). (2) `CheckoutForm` extracts `submittedValues` from state and calls `reset(submittedValues)` in a `useEffect` keyed on `state`. Covers all fields: scalar text (email, firstName, lastName, phone), the `attendeeNames` array, the `needsInvoice` checkbox, invoice optional fields (companyName, nip, invoiceAddress), acceptedTerms, and the four hidden fields (ticketTypeId, quantity as number, locale, currency). Two new action tests written and confirmed failing before the fix; 2 failed, 7 passed. After the fix: 9/9. Negative control: removing `values` from the soldOut return caused exactly `echoes submitted values alongside a _form-level error` to fail. Restored. `CheckoutForm` itself has no test coverage (environment: node, no DOM); repopulation behaviour verified by code reading only. Suite: **406 tests / 45 files, all green.** `pnpm typecheck` and `pnpm lint` both exit 0. |

| **7 Sep (UX defect — post-redirect race, FIXED)** | After paying by card the buyer was redirected from Stripe back to the order page with the Pay button still active. Clicking it returned "tickets already paid". **Cause:** `return_url` redirect arrives faster than the webhook. In that window `Order.paymentIntentStatus` still reads `'requires_confirmation'` (written at PI creation), which the band computation in `order-lookup.ts:112–119` did not treat as in-flight; it fell through to `holdExpiresAt` → `holding` → rendered the Pay button. **Fix:** when `status === 'PENDING' && stripePaymentIntentId != null`, call `stripe.paymentIntents.retrieve()` for the live status, then override the band: `succeeded / processing / requires_action / requires_capture` → `processing` band (reuses the polling island, no new polling mechanism); `canceled` → `cancelled`; `requires_payment_method / requires_confirmation` → keep stored-mirror band so a buyer who abandoned without entering card details can still retry or see the hold-expired message. Guard is structural (`if order.status === 'PENDING' && stripePaymentIntentId != null`) with an inline comment forbidding hoisting. If Stripe throws, falls back to stored-mirror and logs at `console.error` — page never errors. `stripePaymentIntentId` added to the DB select. The `processing.body` i18n copy ("Twoja płatność jest przetwarzana. Ta strona odświeży się automatycznie.") is sensible for the post-redirect case; no new i18n keys added. **Tests:** 5 new cases in `tests/lib/server/order-lookup-stripe.test.ts` (new file, 46th); confirmed 1 failed before fix (`PENDING + PI succeeded → processing band (the post-redirect race)`) and all 5 passed after. Negative control: disabled the guard with `if (false && ...)`, the named test failed. Restored. Suite: **411 tests / 46 files, all green.** `pnpm typecheck` and `pnpm lint` both exit 0. |

| **7 Sep (waluta — decyzja właściciela, WYKONANO)** | Otwarta decyzja dotycząca widoczności waluty rozstrzygnięta 7 Sep 2026. **Nagłówkowy `CurrencySwitcher` pozostaje bez zmian.** Dropdown waluty dodany wyłącznie do `BuyBox` (punkt zakupu) z linią metod płatności pod spodem (`payingIn`). `paymentMethodLabel` i `paymentMethodLabelsFor` wyekstrahowane jako czyste funkcje do `src/lib/shared/payment-methods.ts`, pokryte 9 nowymi testami w `tests/lib/shared/payment-methods.test.ts`. **PayPal (tylko EUR) nie podlega ograniczeniom SEPA** — rozlicza się w sekundy jak karta; ograniczenia (cap współbieżnych holdów, ukrywanie przy wyprzedaniu) dotyczą tylko SEPA, które rezerwuje miejsca na dni. Decyzja właściciela 7 Sep 2026. Komponenty nie są pokryte testami (środowisko: node, brak DOM). | Zarejestrowano w `STATUS.md`. |

| **7 Sep (Task 15 prep, MEASURED)** | **`plan/DEPLOY-PLAN-05.md` did not exist**, although `HANDOFF.md` referenced it twice — once as the step-by-step for Task 15, once as the pre-release checklist ("run Part 3 before every release that touches checkout"). It is in no commit (`git log --all -- plan/DEPLOY-PLAN-05.md` is empty), so the owner had been pointed at a missing document since 6 Sep. Reading the repo to write it surfaced a second defect: **`HANDOFF.md`'s "Where the crons run" table named routes that do not exist.** Actual paths are `/api/cron/release-holds`, `/api/cron/async-release-holds`, `/api/cron/reconcile` (not `sweep-primary` / `sweep-async`); actual schedules are `*/5`, `*/30`, `*/10` (documented as 5, 5, 15); and the documented `RECONCILE alerts=<n> failed=<f> deadlettered=<d>` format does not match `reconcile.ts:164`, which emits `RECONCILE recovered=<r> stuckWebhooks=<w> ticketGaps=<g> alerts=<a>` — so the prescribed `deadlettered=[1-9]` and `failed=[1-9]` greps could never have matched a RECONCILE line. Every alert-watching instruction in the handoff was therefore inoperative. | `plan/DEPLOY-PLAN-05.md` written from Task 15 + Task 16 Step 7 + the 7 Sep traps, verified line by line against `vercel.json`, `src/app/api/cron/*/route.ts`, `src/app/api/webhooks/stripe/route.ts`, `src/lib/server/env-schema.ts`, `src/lib/server/webhook-dispatch.ts` and `src/lib/shared/payment-methods.ts`. `HANDOFF.md`'s cron table and alert-format block corrected, with a dated note recording what was wrong. |

| **7 Sep (P2025 flake, MEASURED — still open)** | Another attempt at the unresolved `sweep-holds.test.ts` P2025 flake, this time under the condition that produced it originally: clean-tree gate (47 files, 427/427, exit 0) followed immediately by a **second `pnpm test` against the now-warm `km_test`**. Result: **427/427 again, exit 0, no P2025.** | Nothing to close. Warm-database-on-second-run is now also insufficient to reproduce it, which narrows the trigger further without identifying it. The row above stands: if it recurs, **capture the failing run's full output before re-running.** |

| **7 Sep (Task 15 verified + demo blocker, MEASURED)** | Owner completed Task 15. Verified live against `https://tickets-km.vercel.app`: `/api/cron/release-holds` → **401 `unauthorized`**, `POST /api/webhooks/stripe` → **400 `missing stripe-signature`**, `/pl` → **200**. So the env vars are set, the build shipped with the cron routes, and `CRON_SECRET` parsed. **But the shop renders "W tej chwili nie ma koncertów w sprzedaży".** `verify-holds` against the Neon connection string reported `0 ticket types`, and a direct count confirmed **Neon `development` holds 0 venues, 0 concerts, 0 ticket types, 0 orders** — the deployed site is correct and empty. **The six-flow walkthrough cannot run until content is seeded**, which is a Plan 02 Task 9 assumption ("dummy data is seeded there") that was never actually carried out. | Blocker recorded. Seeding path built and documented — see the next row — and written into `HANDOFF.md` under Environments. |
| **7 Sep (`.env.neon` points at PRODUCTION, MEASURED — near miss)** | `HANDOFF.md` listed `.env.neon` as "Neon connection strings" without saying **which branch**. It is **`production`**: querying through it returned exactly the two real admin accounts (`mateusz.rusowicz@`, `mde@`) and zero content. The obvious next move for anyone trying to fill the empty demo site — `pnpm exec dotenv -e .env.neon -- pnpm db:seed` — would therefore have written 11 dummy concerts **and two admin accounts with the published password `DevPassword123!` into the production branch**, on an account with no monitoring and an untested restore. The instruction against it existed in `CLAUDE.md` and `HANDOFF.md` but was documentation only. *(The `verify-holds` call that surfaced this was read-only — no `--fix` — so nothing was written.)* | **Enforced instead of documented.** New `src/lib/shared/seed-guard.ts` (`isLocalDatabase`, `adminSeedDecision`) — admin seeding is allowed only against `localhost`/`127.0.0.1`/`::1`; a remote target must pass `SEED_SKIP_ADMINS=1` for content-only, or the seed **throws before writing a row** (decision made at the top of `main()`, not the end). 10 new tests in `tests/lib/shared/seed-guard.test.ts` + 1 in `tests/prisma/seed.test.ts`; the seed test was confirmed failing first (`expected 2 to be +0`). End-to-end control run: remote+no flag → refuses; local+no flag → `2 admin accounts`; local+flag → `NO admin accounts (SEED_SKIP_ADMINS=1)`. `pnpm db:seed:remote` added. `CLAUDE.md` and `HANDOFF.md` corrected to name the branch each env file points at. |

| **17 Sep (MEASURED)** | The 7 Sep Task 15 verification was right that Plan 05 shipped, but incomplete about *which* Plan 05. `origin/development` (what Vercel deploys) is PR #5, merged 6 Sep, ending at Task 14; `git log origin/development..origin/feat/plan-05-payments` = `96260fb` alone. So the live site predates every fix from the 7 Sep walkthrough — form-values echo, post-redirect band, `recordPaymentAttempt` on `succeeded`, currency dropdown, PayPal. The smoke curls could not tell: the routes they hit are identical in both builds. `96260fb` has no migration. Live shop still empty; `.env.neon-dev` never created. | Demo prerequisites are now three, in order: commit the 7 Sep tree, merge `feat/plan-05-payments` → `development`, seed the demo DB content-only. Recorded in `STATUS.md`. |

| **17 Sep (demo DB seeded — live site reads a different database, MEASURED, open)** | `.env.neon-dev` created by the owner from Neon's Connect dialog (branch `development`). Checked without printing values: both keys present, pooled/direct hosts correct, **different endpoint and different Neon timeline** from `.env.neon`, same project, neither a read replica. Before seeding, both branches were **byte-identical in what was checked**: 0 content, the same two real admins, the same 2 `AdminSession` rows (latest 27 Aug), and all 5 migrations with **identical `finished_at` to the second** (`09-04 06:12:11`, `09-06 09:34:44`) — so one branch was copied from the other after 6 Sep. *(This also means the 7 Sep evidence that `.env.neon` is production — "it holds the two real admins" — was not discriminating; development holds them too. The conclusion stands only because the project has two branches and the owner selected `development` for the other file.)* `pnpm db:seed:remote` → `Seeded 2 venues, 11 concerts, NO admin accounts`. Afterwards: development `venues=2 events=11`, production `venues=0 events=0` (untouched). **The live shop still renders "nie ma koncertów w sprzedaży" in all three locales, with `x-vercel-cache: MISS`, `age: 0`.** So Vercel Production is **not** reading Neon `development`, contrary to `HANDOFF.md` ("Production → Neon `development` until launch"). Most likely it reads `production`, which would also explain who applied migrations 4 and 5 there — but production is empty too, so the page cannot distinguish them. | **Open.** Identify with a harmless write: owner logs into `/admin/login` on the live site; whichever branch gains an `AdminSession` row is the one Vercel uses. Do not seed production to "make the demo work" — if confirmed, repoint Vercel's `DATABASE_URL`/`DIRECT_URL` (Production + Preview) at development and redeploy. |

| **17 Sep (RESOLVED — Vercel Production reads Neon `production`, MEASURED)** | Owner logged into `/admin/login` on the live site at 15:02 UTC. Read-only count immediately after: **production** `AdminSession` 2 → **3**, newest `2026-09-17 15:02:32`; **development** unchanged at 2, newest `2026-08-27`. So Vercel Production's `DATABASE_URL` points at Neon **`production`**, contradicting `HANDOFF.md`'s "Production → Neon `development` until launch" (decided 27 Aug). Consequences: the 4 Sep and 6 Sep Vercel builds applied Plan 04's and Plan 05's migrations to the real production branch (additive, harmless); the live shop has always read the database meant to stay dormant until Plan 02 Task 9; and every Stripe test order placed on the live URL would have landed there. | Fix is a Vercel change, not a data change: set `DATABASE_URL` and `DIRECT_URL` for **Production and Preview** to the Neon `development` values (the owner now has them in `.env.neon-dev`), redeploy, and confirm concerts appear. **Do not seed production.** `HANDOFF.md` Environments table annotated. |

| **17 Sep (Vercel repointed to `development`, MEASURED)** | Owner overwrote `DATABASE_URL` + `DIRECT_URL` for Production and Preview with the `development` values and redeployed. Live, with cache-busting query: `/pl`, `/de`, `/en` each list 8 concert links; `<p>W tej chwili nie ma koncertów` count **0** (the earlier grep hit was the next-intl message payload, not rendered copy). Concert page shows `<select id="buybox-currency">` and `Płacisz w PLN m.in.: karta, BLIK, Przelewy24` → `96260fb` deployed. `hreflang` links are `https://tickets-km.vercel.app/…`, zero `localhost` → **`NEXT_PUBLIC_SITE_URL` confirmed**, which Task 15 Step 7 had only ever checked by eye. Production `events=0 orders=0`; development `events=11 orders=1`. | Blocker closed. Task 16 Step 7 unblocked. |

| **17 Sep (writing `05-manual-test.md`, VERIFIED against Stripe docs)** | Two assumptions corrected while collecting test data. **(1) Klarna does not need a real phone number** — Stripe publishes per-country Klarna sandbox identities (DE approved: `customer@email.de`, `+49017614284340`; OTP any 6 digits, `999999` fails). `HANDOFF.md`'s "needs a real phone number, so it was never exercised" was wrong; Klarna is testable. **(2) BLIK failure simulation is probably unreachable from our checkout.** Stripe triggers BLIK declines/timeouts by the **payment method's billing e-mail** (`customer_declined@…` etc.). `PaymentElementIsland` confirms without `billing_details`, and the order e-mail reaches Stripe only as the PaymentIntent's `receipt_email`, a different field. Not code-verified against a live confirm — scenario B6 checks it once. | `plan/05-manual-test.md` written (scenarios A–E + all test data, sourced). `DEPLOY-PLAN-05.md` Part 3.1 now points to it instead of duplicating the flows. `HANDOFF.md` Klarna paragraph corrected. |
...
|---|---|---|
| `payment_intent.succeeded` | expand `latest_charge` via retrieve, then `recordPaymentAttempt(orderId, pi, { reason: 'succeeded' })`, then `fulfilOrder(orderId, refund, pi)` | PENDING → PAID (or REFUNDED via late branches) |
...
> **Do this from [`../DEPLOY-PLAN-05.md`](../DEPLOY-PLAN-05.md), not from the
> steps below.** That runbook expands this task with the exact env-var table,
> the smoke curls with their expected bodies, the Link and PayPal dashboard
> toggles found on 7 Sep, and the sandbox trap that must be checked before the
> walkthrough. The steps here are kept as the plan's own record.
